#+TITLE:   Steel Beach Common Lisp -- Robert Strandh's "Fast Generic Dispatch" for SBCL
#+AUTHOR:  Jan Moringen
...
** TODO ~vector~ Specializer

   As soon as a ~vector~ specializer is involved, PCL seems to switch
   to the (generally much slower) "discrimination net"-based dispatch,
   although a ~vector~ layout exists. Investigate why. Does this
   affect our approach?
